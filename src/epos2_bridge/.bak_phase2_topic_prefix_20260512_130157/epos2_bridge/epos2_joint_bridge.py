...
IDX_INTERPOLATION_STATUS = 0x20C4
IDX_INTERPOLATION_DATA_CONFIG = 0x60C4
...
class Epos2JointBridge(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("traj_internal_max_dv_rad_s", 0.35)
...
# ---------------- Timers ----------------
    ...
    def _clear_fault_srv(self, request, response):
        ok = self.clear_fault()
        if ok:
            self._set_bridge_state(BridgeState.READY, "fault cleared by service")
        else:
            self._set_bridge_state(BridgeState.FAULTED, "clear_fault service failed")
        response.success = ok
        response.message = "clear_fault succeeded" if ok else "clear_fault failed"
        return response
    ...
    def _disarm_ipm_srv(self, request, response):
        ok = self.disarm_ipm()
        response.success = ok
    ...
    def send_test_move_absolute(self, target_rad: float) -> bool:
        ...
        if current_pos is None:
            self.get_logger().error("Failed reading current position for absolute move")
            return False

        current_pos_qc = to_signed_32(current_pos)
        current_rad = self.kin.motor_qc_to_joint_rad(current_pos_qc)
        delta_rad = float(target_rad) - float(current_rad)

        self.get_logger().info(
            f"Absolute move request target_rad={target_rad:.6f} current_rad={current_rad:.6f} delta_rad={delta_rad:.6f}"
        )
    ...
    def _move_delta_srv(self, request, response):
        ...
        response.success = ok
        response.message = (
            f"move_delta succeeded for delta_rad={request.delta_rad}"
            if ok else
            f"move_delta failed for delta_rad={request.delta_rad}"
        )
        return response
    ...
    def _move_absolute_srv(self, request, response):
        ...
        response.success = ok
        response.message = (
            f"move_absolute succeeded for target_rad={request.target_rad}"
            if ok else
            f"move_absolute failed for target_rad={request.target_rad}"
        )
        return response
    ...
    def clear_and_enable_ipm_buffer(self) -> Tuple[bool, bool, Optional[int]]:
        ok0 = self.sdo_write(IDX_INTERPOLATION_DATA_CONFIG, 6, 0, warn=False)
        time.sleep(0.01)
        ok1 = self.sdo_write(IDX_INTERPOLATION_DATA_CONFIG, 6, 1, warn=False)
        time.sleep(0.01)
        ...
        if ipm_buf is not None:
            ...
            self.get_logger().warning("Could not read 0x20C4:01 after buffer clear/enable")
    ...
    def enter_position_mode_hold(self, hold_qc: Optional[int] = None) -> bool:
        """Hold between trajectories in Maxon Position Mode 0xFF using 0x2062."""
        try:
            if hold_qc is None:
            ...
            # Drop IPM-active bit 4 and keep the power stage Operation Enabled.
            self.sdo_write(IDX_CONTROLWORD, 0, 0x000F, warn=False)
            try:
                self.command_control_mode(0x000F, MODE_INTERPOLATED_POSITION, "prepare_or_clear_ipm_bit4_low")
            except Exception:
            ...
            ok = True
            ok &= self.sdo_write(IDX_POSITION_MODE_SETTING_VALUE, 0, hold_qc, warn=False)
            ok &= self.sdo_write(IDX_MODES_OF_OPERATION, 0, MODE_MAXON_POSITION, warn=False)
            try:
                self.command_control_mode(0x000F, MODE_MAXON_POSITION, "enter_maxon_position_mode_hold")
            except Exception:
                pass
            time.sleep(0.05)
            ok &= self.sdo_write(IDX_CONTROLWORD, 0, 0x000F, warn=False)
            ...
            self.get_logger().info(
                ...
                f"sw=0x{((sw or 0) & 0xFFFF):04X} mode=0x{((mode_disp or 0) & 0xFF):02X} ok={ok}"
            )
            return bool(ok)
        except Exception as exc:
    ...
    def terminate_ipm_and_enter_position_hold(self, target_qc: int, final_dt_ms: int = 20) -> bool:
        """Official IPM exit: optional final zero-velocity point, dt=0 terminator, then 0xFF hold."""
        target_qc = to_signed_32(target_qc)
        final_dt_ms = max(1, min(255, int(final_dt_ms)))
        try:
            # A final normal record makes the last commanded target unambiguous before dt=0.
            ...
            # Maxon IPM terminator: delta-T/time byte zero.
            self.send_rpdo1_interpolation_record(
            ...
            # Explicitly clear IPM-active bit 4 as the belt-and-suspenders path.
            self.sdo_write(IDX_CONTROLWORD, 0, 0x000F, warn=False)
            try:
                self.command_control_mode(0x000F, MODE_INTERPOLATED_POSITION, "prepare_or_clear_ipm_bit4_low")
            except Exception:
            ...
            # Short poll: bit 15 low means IPM inactive. Do not fail solely on this; switch hold anyway.
            ...
            while time.monotonic() < deadline:
                ...
                if ipm is not None:
                    ...
                    if not ((last_ipm >> IPM_ACTIVE) & 0x1):
                        break
                time.sleep(0.02)
            self.get_logger().info(f"IPM terminator observed status=0x{((last_ipm or 0) & 0xFFFF):04X}")

            return self.enter_position_mode_hold(target_qc)
        except Exception as exc:
            self.get_logger().error(f"terminate_ipm_and_enter_position_hold() exception: {exc}")
    ...
    def startup_ipm(self) -> bool:
        ...
        try:
            ...
            if sw0 is not None and ((sw0 >> SW_FAULT_BIT) & 0x1):
                if not self.clear_fault():
                    self.get_logger().error("clear_fault failed during startup_ipm")
                    self._set_bridge_state(BridgeState.FAULTED, "clear_fault failed")
                    return False
                time.sleep(0.05)
            ...
            if not self.enable_operation():
                self.get_logger().error("enable_operation failed")
                self._set_bridge_state(BridgeState.FAULTED, "enable_operation failed")
                return False
            ...
            if ipm_pre is None:
                ...
                ipm_pre_hex = f"0x{ipm_pre:04X}"
            self.get_logger().info(
                f"clear_and_enable_ipm_buffer ok0={ok0} ok1={ok1} ipm={ipm_pre_hex}"
            )

            current_pos = self.sdo_read(IDX_POSITION_ACTUAL, 0, warn=False)
            if current_pos is None:
                self.get_logger().error("Failed reading current position before staged holds")
            ...
            hold = PVTPoint(time_ms=seg_ms, velocity_rpm=0, position_qc=current_qc)
            ...
            if not self.enable_ipm_active():
                self.get_logger().error("Failed to activate interpolation mode")
                self._set_bridge_state(BridgeState.FAULTED, "enable_ipm_active failed")
                return False
            ...
            ipm_hex = f"0x{ipm_buf:04X}" if ipm_buf is not None else "None"
            self.get_logger().info(f"startup_ipm final sw={sw_hex} ipm={ipm_hex}")

            if sw is not None and ((sw >> SW_FAULT_BIT) & 0x1):
                self.get_logger().error(f"Drive faulted during IPM arm, sw={sw_hex}")
            ...
            if ipm_buf is None:
                ...
                self._set_bridge_state(BridgeState.FAULTED, "no ipm status after arm")
                return False

            if not ((ipm_buf >> IPM_BUF_ENABLED) & 0x1):
                self.get_logger().error(f"IPM buffer not enabled after arm, ipm={ipm_hex}")
                self._set_bridge_state(BridgeState.FAULTED, "buffer not enabled")
                return False

            if not ((ipm_buf >> IPM_ACTIVE) & 0x1):
                self.get_logger().error(f"IPM did not become active after arm, ipm={ipm_hex}")
                self._set_bridge_state(BridgeState.FAULTED, "ipm not active")
                return False

            self.ipm_armed = True
            self._set_bridge_state(BridgeState.IPM_ARMED, "IPM active and buffered")
            self.get_logger().info(f"IPM armed successfully, sw={sw_hex} ipm={ipm_hex}")
            return True
    ...
    def disarm_ipm(self) -> bool:
        # Stop feeding the FIFO first.
        self.ipm_armed = False

        # Return to normal enabled state, but not interpolation-active.
        ok = self.sdo_write(IDX_CONTROLWORD, 0, 15)
        time.sleep(0.05)
        ...
        self.get_logger().info(
            f"disarm_ipm sw=0x{((sw or 0) & 0xFFFF):04X} "
            f"ipm=0x{((ipm_buf or 0) & 0xFFFF):04X}"
        )

        if sw is not None and ((sw >> SW_FAULT_BIT) & 0x1):
            self._set_bridge_state(BridgeState.FAULTED, "fault during disarm")
            return False

        self._set_bridge_state(BridgeState.READY, "IPM disarmed")
        return ok
    ...
    def send_test_move_delta(self, delta_rad: float) -> bool:
        ...
        if self.bridge_state not in (BridgeState.IPM_ARMED, BridgeState.MOVING):
            self.get_logger().warning(f"Refusing move in bridge state {self.bridge_state.name}")
            return False
        ...
        self._set_bridge_state(BridgeState.MOVING, "queueing test move")
        ...
        try:
            ...
            # Give it a moment to settle back into hold
            time.sleep((3.0 * seg_ms) / 1000.0 + 0.05)

            sw = self.sdo_read(IDX_STATUSWORD, 0, warn=False)
            if sw is not None and ((sw >> SW_FAULT_BIT) & 0x1):
                self._set_bridge_state(BridgeState.FAULTED, "fault after test move")
                self.ipm_armed = False
                return False

            self._set_bridge_state(BridgeState.IPM_ARMED, "returned to hold after move")
            return True

        except Exception as exc:
            self.get_logger().error(f"Failed sending test move: {exc}")
            self._set_bridge_state(BridgeState.FAULTED, "exception during test move")
            self.ipm_armed = False
            return False
    ...
    def _segment_points_between(
        ...
        for k in range(1, n_segments + 1):
            ...
            if abs(seg_delta_qc) <= 2:
                motor_rpm = 0
                seg_target_qc = prev_qc
            else:
                motor_rpm = int(
                    math.ceil(
                        abs(seg_delta_qc) * 60.0
    ...
    def _shape_preserving_knot_velocities(
        self,
        knots_rad: List[float],
        durations_sec: List[float],
        measured_vel_rad_s: float,
    ) -> List[float]:
        """
        Estimate knot velocities from neighboring segment slopes using a
        shape-preserving PCHIP-like rule.
        """
        n_seg = len(durations_sec)
        if len(knots_rad) != n_seg + 1:
            raise ValueError("knots_rad must have len(durations_sec)+1")

        slopes = []
        for i in range(n_seg):
            dt = max(1e-6, float(durations_sec[i]))
            slopes.append((knots_rad[i + 1] - knots_rad[i]) / dt)

        v = [0.0] * (n_seg + 1)

        if n_seg > 0 and measured_vel_rad_s * slopes[0] > 0.0:
            vmax0 = 1.5 * abs(slopes[0])
            v[0] = max(-vmax0, min(vmax0, measured_vel_rad_s))
        else:
            v[0] = 0.0
        ...
        for i in range(1, n_seg):
            s_prev = slopes[i - 1]
            s_next = slopes[i]
            if s_prev == 0.0 or s_next == 0.0 or (s_prev * s_next) <= 0.0:
                v[i] = 0.0
            else:
                h_prev = max(1e-6, float(durations_sec[i - 1]))
                h_next = max(1e-6, float(durations_sec[i]))
                w1 = 2.0 * h_next + h_prev
                w2 = h_next + 2.0 * h_prev
                v[i] = (w1 + w2) / ((w1 / s_prev) + (w2 / s_next))
    ...
    def _hermite_pos_vel_acc(
        ...
        v = (dh00*q0 + dh10*T*v0 + dh01*q1 + dh11*T*v1) / T
        a = (d2h00*q0 + d2h10*T*v0 + d2h01*q1 + d2h11*T*v1) / (T*T)
        return q, v, a

    def _segment_points_between_adaptive(
        self,
        q0_rad: float,
        q1_rad: float,
        v0_rad_s: float,
        ...
        dt_sub = T / n
        time_ms = max(min_seg_ms, min(255, int(round(dt_sub * 1000.0))))
    ...
    def execute_reduced_trajectory(self, target_rads: List[float], duration_secs: List[float]) -> bool:
        ...
        with self.stream_lock:
            ...
            current_pos = self.sdo_read(IDX_POSITION_ACTUAL, 0, warn=False)
            current_vel = self.sdo_read(IDX_VELOCITY_ACTUAL, 0, warn=False)
            if current_pos is None:
                self.get_logger().error("Failed reading current position for reduced trajectory")
                return False
            ...
            current_vel_rpm = to_signed_32(current_vel or 0)
            current_vel_rad_s = self.kin.motor_rpm_to_joint_rad_s(current_vel_rpm)
            if abs(current_vel_rad_s) < 0.15:
                current_vel_rad_s = 0.0

            durations = [max(0.03, float(d)) for d in duration_secs]
            knots = [current_rad] + [float(q) for q in target_rads]
            knot_vels = self._shape_preserving_knot_velocities(knots, durations, current_vel_rad_s)

            all_points: List[PVTPoint] = []
            for i in range(len(target_rads)):
                q0 = knots[i]
                q1 = knots[i + 1]
                v0 = knot_vels[i]
                v1 = knot_vels[i + 1]
                dt = durations[i]
                all_points.extend(self._segment_points_between_adaptive(q0, q1, v0, v1, dt))
            ...
            if len(all_points) > max_total_points:
                keep_idxs = sorted({
                    round(i * (len(all_points) - 1) / (max_total_points - 1))
                    for i in range(max_total_points)
                })
                all_points = [all_points[i] for i in keep_idxs]
                self.get_logger().info(
                    f"Capped adaptive resampling to {len(all_points)} PVT points"
                )

            self.get_logger().info(
                f"Adaptive bridge resampling: {len(target_rads)} reduced knots -> {len(all_points)} PVT points"
            )
    ...
    def send_rpdo2_control_mode(self, controlword: int, mode: int) -> None:
        ...
        self.can.send(self.cob_rpdo2, data)
    ...
    def _poll_slow_state_and_publish(self) -> None:
        ...
        s.data = (
            ...
            f"bridge_state={self.bridge_state.name} "
    ...
    def _disarm_ipm_cb(self, msg: Bool) -> None:
        if not msg.data:
            return
        ok = self.disarm_ipm()
        self.get_logger().info(f"/disarm_ipm_now result={ok}")
        ...
        try:
            while record_idx < len(all_pvt):
                if goal_handle.is_cancel_requested:
                    ...
                    try:
                        if record_idx > 0:
                            cancel_hold_qc = all_pvt[max(0, record_idx - 1)].position_qc
                        else:
                            with self.state_lock:
                                cancel_hold_qc = self.state.position_actual_qc
